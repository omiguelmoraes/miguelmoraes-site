...
- Criação e refinamento da Landing Page de tráfego pago `/trabalhe-comigo` (`src/pages/trabalhe-comigo.astro`):
  ...
  - Reformulação do painel de marcas: substituição da grade fechada por cards brancos individuais com gap ampliado para 18px. Ajuste do título para "Empresas com quem já trabalhei" (posicionamento honesto e preciso) e ampliação do espaçamento vertical (`72px 0 84px`). No mobile, limitação às 6 principais empresas em 2 colunas para respiro visual.
  - Refino da Hero: layout horizontal no pill badge com indicador verde de agenda aberta (`.status-tag`), remoção do chavão genérico "Design, marketing e IA" e nova sub-headline focada em entregáveis de valor (Landing Pages, sites e automações).
  - Reescrita estratégica de posicionamento na seção "Quem executa" e FAQ: restauração do texto sênior de autoridade (*"Sou o Miguel Moraes, designer e especialista digital em Porto Alegre. Desenvolvo a estratégia visual e a experiência de produtos digitais, dashboards e sites que movimentam dezenas de milhares de usuários"* e *"Eu lidero pessoalmente a estratégia, a arquitetura e o design... mobilizo parceiros seniores de extrema confiança"*).
  - Ajuste de UX no formulário e Dock flutuante: botão do formulário expandido para 100% da largura (`width: 100%`) e CTA flutuante simplificado para "Precisa de um projeto sob medida?" em linha única com `white-space: nowrap`, mantendo o botão "Solicitar orçamento" limpo.
  - Expansão de escopo para Produtos Digitais & Dashboards: atualização global da oferta (Hero, FAQ, formulário e dock) para abranger também redesenho e UI/UX de sistemas internos, ferramentas web e dashboards.
  - Transformação da seção "Soluções": substituição da lista textual simples por cards visuais em grade 2x2 contendo ilustrações vetoriais mínimas em SVG (browser layout, dashboard KPI chart, workflow de automação e gráfico de métricas).
  - Tom de Copy Incisivo (Estilo Supertape / Foco Organico & SEO): reescrita da Hero com foco em conversão e busca ("*Seu site atrai visitantes e deixa dinheiro na mesa. Transformo acessos em clientes pagantes. Estrutura otimizada para ser achado no Google, citado em pesquisas de IA e converter...*").
  - Simplificação da Bio & Card de Oferta: parágrafo "Quem executa" enxugado para 2 frases diretas, remoção do texto de prazo acima do preço no card de oferta (ficando restrito ao FAQ) e substituição da promessa de "painel auto-editável" por *"Código moderno, seguro e sem dependência de plugins pesados"*.
  - Otimização da Grade de Ofertas no Mobile: correção da media query para empilhar os 2 cards de oferta verticalmente em 1 coluna única (`grid-template-columns: 1fr`) no mobile, eliminando a compressão lateral.
  - Reestruturação da Seção de Investimento (Modelo Korbi): divisão da oferta em grade de 2 cards (Card 1: *Construção de Site ou LP* a partir de R$ 2.000 taxa única; Card 2: *Motor de Crescimento* opcional a partir de R$ 1.000/mês destacando *"Criação e gerenciamento de conteúdo para blog automatizado e focado em SEO/GEO"* sem fidelidade).
  - Polimento Estético High-End SaaS: fundo em grid sutil no Hero, bordas multicamadas em cards e steps, animação de pulso no status tag verde, cards de oferta com destaque visual no Motor e tipografia ajustada estilo Linear/Vercel.
  - Padronização de Espaçamentos Verticais nas Seções: alinhamento global de `.sec` para `padding: 72px 0` no desktop e `padding: 48px 0` no mobile, garantindo que o topo e a base de todas as seções (incluindo trabalhos recentes) tenham ritmo visual 100% simétrico e idêntico.
  - Overhaul do Rodapé (SaaS High-End 3 Colunas): reestruturação em 3 colunas com avatar/bio, status ao vivo, punchline *"Menos slop. Mais design."*, background sutil (`var(--wash-subtle)`), links limpos (Instagram & LinkedIn) e correção do URL oficial do LinkedIn (`https://linkedin.com/in/miguelmoraes/`) em todo o projeto.
  - Ajuste de Ritmo no Formulário e Esteira: respiro superior da seção de formulário reduzido (`48px`) e respiro inferior ampliado (`80px`), além de espaçamento expandido entre o título "Alguns projetos." e os cards da esteira (`52px`), removendo também a linha divisória sobre o texto do veredito.
  - Inclusão de `text-wrap: balance` e `text-wrap: pretty` nos títulos e parágrafos para eliminar palavras viúvas em todas as resoluções.
...
- Redesign e simplificação da página `/links` (`src/pages/links.astro`): remoção do card de produto em destaque e da seção "Vida recente", padronizando Mentoria, PackPro e Calculadora em uma lista única de produtos com links diretos.
- Criação da Landing Page de Mentoria (`/mentoria`): nova página em `src/pages/mentoria.astro` com visão geral do acompanhamento 1-on-1 para designers/freelancers (posicionamento, processos e técnica) e formulário integrado de captação de interesse direcionado para o WhatsApp.
